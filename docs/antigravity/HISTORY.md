...
### 用户体验与展示优化 (v2.2.0)
- **视觉体验升级**:
    - **全屏沉浸模式**: Web 端图片/视频查看器新增全屏切换功能，支持 ESC 键退出，提供无干扰的浏览体验。
    - **智能递归封面**: 优化了后端 `findCoverMedia` 算法。当当前文件夹没有媒体文件时，系统会深度递归其所有子目录下探查找首个可用媒体文件作为封面，彻底消灭了“有内容但封面为空”的视觉断层。
- **工程化与构建**:
    - **Docker构建加速**: `Dockerfile` 集成了 npm 淘宝镜像源配置，解决了国内环境下构建超时的问题。
    - **镜像瘦身**: 优化 `.dockerignore` 排除策略，剔除了测试文件与覆盖率报告，确保生产镜像的纯净与轻量。
